...
int Activate(int port){
    ...
    iResult = send(connectSocket,i , (int)strlen(i) + 1, 0);
    ...
    printf("****Activation message sent: %s****\n",i);
...
int RecvData(int port){
    ...
    do
    {
        ...
        if (iResult > 0)
        {
            ...
            bool a=WriteInTxt(recvbuf, port);

            if (a == true) {
                printf("Message succesfully added to txt file\n");
            }
            else {
                printf("Error occured while saving message!\n");
            }
